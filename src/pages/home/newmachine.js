...
class NewMachine extends Component {
    ...
    loadInfo = () => {
        var that = this;
        locals.PrepareFormType("NewMachine")
            .then(data => {
                let _dataList = [];
                for (var key in data) {
                    if (data[key].value !== '') {
    ...
    Save = () => {
        ...
        newdata.push({ "Key": "ActionType", "Value": "NewMachine" });
    ...
    render() {
        return (<View style={[commonStyle.safeViewWithCusHead, { backgroundColor: '#fff' }]}>
            <CustomizeHeader showBack={true} Title="新机器" goBack={() => { this.props.navigation.goBack() }}>
            </CustomizeHeader>
