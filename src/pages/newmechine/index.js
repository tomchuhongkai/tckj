...
@inject('store')
@observer
class NewMechineList extends Component {
        ...
        return (
            <TouchableOpacity onPress={() => {
                this.props.navigation.push('Detail', {
                    id: item.id, callBack: this.changeRowItem
                })
            }}>
                <View style={[commonStyle.postshow, styles.ershouji]}>
                    ...
                    <View style={styles.ershoujicontent}>
                        ...
                        {item.pictures == undefined || item.pictures.length == 0 ? null : <View style={commonStyle.postImgs}>
                            {item.pictures.map((pic, subIndex) => {
                                return (
                                    // item.pictures.length==1?commonStyle.postImg1:commonStyle.postImg
        ...
        return (<SafeAreaView style={[commonStyle.safeViewWithCusHead, { backgroundColor: '#fff', paddingTop: scaleSizeW(90) }]}>
            ...
            <CustomizeHeader showBack={false} style={{ paddingLeft: 0, width: '100%', backgroundColor: '#4576f7', justifyContent: 'flex-start' }}>
                ...
                <RightButton children={<TouchableOpacity
                    onPress={() => that.props.navigation.navigate('PostNewMachine', {
                        callBack: () => {
                            that.loadData({ page:1 }, true);
                        }
                    })}><View><Image style={styles.homeadd} source={require('../../../images/icon-add.png')} /></View></TouchableOpacity>} />
            </CustomizeHeader>
        </SafeAreaView >
        )
    }
